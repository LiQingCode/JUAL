...
parser.add_argument('--task', type=str, default='LocalLaplacian', help='TASK')
...
def generate_list(set_name):
    ...
    img_groups = []
    for idx in idxs:
        imgs = glob.glob(os.path.join(PATH, 'guide', SET, 'a{:04d}-*.tif'.format(idx)))
        for full_path in imgs:
            path = os.path.basename(full_path)
            rgb_path = os.path.join('guide', SET, path)
